...
This is the official implementation of our experiments on learning radio map estimation from images and optionally additional unclassified height maps with CNNs, described in "Radio Map Prediction from Images and Application to Coverage Optimization". The paper will appear soon in IEEE Transactions on Wireless Communications, the preprint can be found on [arxiv](https://arxiv.org/abs/2410.17264).

Note that this is an updated version of our repo [RML](https://github.com/fabja19/RML) with the focus shifted to the prediction from images and an additional new application to network optimization ([notebook](coverage_optimization.ipynb)). 
